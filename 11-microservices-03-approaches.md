...
Запуск сборки как по событию, так и по кнопке с указанием параметров возможен через сервис GitLAb CI, там же удобно хранить различные шаблоны и конфигурации, а также существует своя система контроля качества кода (то есть не ттебуется интеграция с Sonarcube, как в случае, например с Jenkins).

В целом при определении наиболее удобногго конвейера под конкретный проект можно воспользоваться сравнительной таблицей:
